...
module.exports = {
	config: {
		...
		description: 'Pensez à un personnage réel ou fictif, je vais essayer de le deviner',
		acessableby: 'everyone'
	},
	run: async (bot, message, args, ops) => {
		if (!message.channel.permissionsFor(bot.user).has('EMBED_LINKS')) return message.channel.send('**Autorisations manquantes - [EMBED_LINKS]!**');
		if (!args[0]) return message.channel.send(`**Quelle catégorie voulez-vous utiliser ? Soit \`${list(regions, 'ou')}\`!**`);
		...
		if (!regions.includes(stringAki)) return message.channel.send(`**Quelle région souhaitez-vous utiliser ? Soit \`${list(regions, 'ou')}\`!**`);
		const current = ops.games.get(message.channel.id);
		if (current) return message.channel.send(`**Veuillez patienter jusqu'à ce que la partie en cours de \`${current.name}\` soit terminée !**`);
		try {
			...
			while (timesGuessed < 3) {
				...
				const embed = new MessageEmbed()
					...
					.setFooter(`Yes/No pour confirmer | Progression - ${Math.round(Number.parseInt(aki.progress, 10))}%`)
				...
				if ((aki.progress >= 90 && !guessResetNum) || forceGuess) {
					...
					if (!guess) {
						await message.channel.send('**Je ne peux penser à personne !**');
						win = true;
						break;
					}
					...
					if (verification === 0) {
						...
						const exmessage = timesGuessed >= 3 || forceGuess ? 'J"abandonne!' : 'Je peux continuer !';
						await message.channel.send(`**Hmm... Est-ce vrai ? ${exmessage}**`);
			...
			if (win === 'time') return message.channel.send('** Je suppose que votre silence signifie que j"ai gagné ! **');
			if (win) return message.channel.send('** Vous m"avez vaincu cette fois ! **');
			return message.channel.send('**Deviné juste une fois de plus ! J"adore jouer avec toi !**');
		} catch (err) {
			ops.games.delete(message.channel.id);
			return message.channel.send(`**Serveur en panne, réessayez plus tard !**`);
		};
	}
};